					...
					<div class="poptip">
						...
						<Select v-model="filterSearch.unit">
							<Option v-for="(item, index) in coinSearchArr" 
										:value="item.unit" 
										:key="item.unit">{{ item.unit==='' ? item.name : item.unit  }}</Option>
						</Select>
					</div>
			...
			<Modal v-model="showModal" :width="500" class="modelInfo">
					<h3 slot="header">订单信息</h3>
						<ul>
							...
							<li><span>手续费：</span>{{ modelInner.fee > 0 ? modelInner.fee :'' }} </li>
      ...
      columnsList: [
        ...
        },
        {
          ...
          width: 105,
					key: "releaseTime",
					render: (h, ctx) => {
						return h('span', {}, !ctx.row.releaseTime ? '-' : ctx.row.releaseTime);
					}
        },
        {
          title: "操作",
